...
public class Explosives_T1 implements Special {
	public Explosives_T1() {
	}
	...
	public void onInteract(final PlayerInteractEvent e, ItemStack item) {
		...
		if (e.getAction().equals(Action.RIGHT_CLICK_BLOCK)) {
			...
			if ((currentDominion == null) || (!currentDominion.hasUpgrade(UpgradeType.PROT_T1))) {

				org.bukkit.Bukkit.getServer().getScheduler().scheduleSyncDelayedTask(MoreItems.getInstance(),
						new Runnable() {
							public void run() {
								e.getClickedBlock().setType(Material.AIR);
								e.getClickedBlock().getWorld().createExplosion(e.getClickedBlock().getLocation(), 3.0F,
										false);
							}
