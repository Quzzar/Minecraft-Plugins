...
public class ChatProcessor {
	...
	public void sendLocalMessage(Player p, Set<Player> recipients, String sendingMessage) {
		...
		for(Player rp : recipients) {
			...
			if(p.getWorld().equals(rp.getWorld())) {
	    		...
	    		if(distance > baseMax+extRange) {
	    			...
	    			Utility.sendPlayerMessage(prefix, p, rp, sendingMessage);
	    			if(rp != p) {sent = true;}
	    		} else {
	    			// Conceal message...
	    			Utility.sendPlayerMessage(prefix, p, rp, concealMessage(distance, sendingMessage));
	    			if(rp != p) {sent = true;}
	    		}
		...
		if(!sent) {
			boolean isSafeZone = DominionsMain.getZoneManager().isSafeClaimed(p.getLocation().getChunk());
			if(isSafeZone) {
